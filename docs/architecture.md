...
- Authorization is workspace-scoped (who can see which events)
- Tenant packs (Hypeluxe) supply templates and branding, not separate databases unless scale demands it later

## Continuous delivery

1. **PR:** typecheck, unit tests (Effect + test Layers), schema migration dry-run, Playwright smoke
2. **Main → staging:** deploy immutable artifact; smoke includes offline create → online sync
...
```text
packages/
  domain/          # Effect schemas + pure use cases
  client/          # UI + local DB + sync adapter
  server/          # Auth, sync, admin APIs
  sync/            # Shared sync protocol types
